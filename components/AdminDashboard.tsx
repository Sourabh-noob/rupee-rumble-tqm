...
import { Save, LogOut, ChevronRight, PlayCircle, Clock, BarChart3, Eye, EyeOff } from 'lucide-react';
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-4 flex justify-between items-center z-10">
        ...
        <div className="flex gap-4">
             <div className="flex bg-slate-100 dark:bg-slate-900 rounded-lg p-1">
                <button onClick={() => setActiveTab('editor')} className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${activeTab === 'editor' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}>Round Editor</button>
                <button onClick={() => setActiveTab('live')} className={`px-4 py-1.5 rounded-md text-sm font-bold transition-all ${activeTab === 'live' ? 'bg-white dark:bg-slate-700 shadow text-green-600 dark:text-green-400' : 'text-slate-500'}`}>Live Control</button>
             </div>
            ...
            <>
                ...
                <div className="flex-1 overflow-y-auto p-8 relative">
                    ...
                    <div className="absolute bottom-6 right-8">
                        <button onClick={handleSave} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white font-bold px-8 py-3 rounded-full shadow-xl shadow-green-900/20 hover:scale-105 transition-transform"><Save size={20} /> Save Changes</button>
                    ...
                    <div className="p-8 space-y-8">
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-6 gap-2">
                                {[1, 2, 3, 4, 5, 6].map(r => (
                                    <button key={r} onClick={() => setLiveRound(r)} className={`py-4 rounded-xl font-bold text-lg border-2 transition-all ${liveRound === r ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'border-slate-200 dark:border-slate-700 text-slate-400 hover:border-slate-300'}`}>{r}</button>
                                ))}
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-6 border border-indigo-100 dark:border-indigo-900/30 flex flex-col items-center justify-center text-center">
                                <div className="text-sm text-indigo-800 dark:text-indigo-300 font-bold mb-3">Launch Next Query</div>
                                <button onClick={() => onStartRound(liveRound, 1)} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-lg font-bold shadow-lg shadow-indigo-500/30 hover:scale-105 transition-transform"><PlayCircle size={20} /> START ROUND</button>
                            </div>
                            <div className={`rounded-xl p-6 border flex flex-col items-center justify-center text-center transition-colors ${isLeaderboardVisible ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-300' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700'}`}>
                                <div className={`text-sm font-bold mb-3 ${isLeaderboardVisible ? 'text-amber-800 dark:text-amber-300' : 'text-slate-500'}`}>Leaderboard Status</div>
                                <button onClick={toggleLeaderboard} className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold shadow-lg transition-all hover:scale-105 ${isLeaderboardVisible ? 'bg-amber-500 hover:bg-amber-400 text-white shadow-amber-500/30' : 'bg-slate-700 hover:bg-slate-600 text-white'}`}>
                                    {isLeaderboardVisible ? <><EyeOff size={20} /> HIDE STANDINGS</> : <><Eye size={20} /> SHOW STANDINGS</>}
                                </button>
                            </div>
                        </div>
                    </div>
