...
import { RotateCcw, Award, Users, TrendingUp, History } from 'lucide-react';
    ...
    <div className="min-h-screen p-4 md:p-8 flex flex-col items-center max-w-5xl mx-auto space-y-12">
      ...
      <div className="text-center space-y-6 pt-12">
        ...
        <div>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-100 dark:bg-indigo-900/40 rounded-full border border-indigo-200 dark:border-indigo-800">
                <Users size={18} className="text-indigo-600 dark:text-indigo-400" />
                <span className="text-lg text-indigo-900 dark:text-indigo-200 font-bold uppercase tracking-widest">{team.name}</span>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl">
        <div className="bg-white dark:bg-slate-800 p-10 rounded-3xl border border-slate-200 dark:border-slate-700 text-center shadow-2xl transition-all group hover:border-yellow-500/50 hover:shadow-yellow-500/10">
            ...
            <div className={`text-6xl font-mono font-black mt-4 group-hover:scale-110 transition-transform ${finalBalance > 0 ? 'text-slate-900 dark:text-white' : 'text-red-500'}`}>
                ₹{finalBalance}
            </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-10 rounded-3xl border border-slate-200 dark:border-slate-700 text-center shadow-2xl transition-all group hover:border-indigo-500/50 hover:shadow-indigo-500/10">
            ...
            <div className="text-6xl font-mono font-black text-indigo-600 dark:text-indigo-400 mt-4 group-hover:scale-110 transition-transform">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden relative">
            <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-bold flex items-center gap-3 text-slate-900 dark:text-white">
                    <TrendingUp size={24} className="text-indigo-500"/> Equity Curve
                </h3>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {team.history.map((round) => (
                            <tr key={round.roundNumber} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td className="p-4 font-mono font-bold text-green-600 dark:text-green-400">
                                  +₹{round.allocations[round.correctAnswer]}
                                </td>
                                <td className="p-4 pr-8 font-mono text-right font-bold text-slate-900 dark:text-slate-100">
                                  ₹{round.endBalance}
                                </td>
                            </tr>
                        ))}
